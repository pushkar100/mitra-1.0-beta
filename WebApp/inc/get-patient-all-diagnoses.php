<?php
	...
	if(	isset($_GET['patientID']) && $_GET['patientID'] ) {	
		...
		$result = getPatientAllDiagnoses ($patientID, $connection);

		if ($result) { //success
			$JSON_RESULT_ARRAY = $result;
			$JSON_RESULT_ARRAY['ERROR'] = 0;
			$JSON_RESULT_ARRAY['SUCCESS_MSG'] = 'All Diagnoses Of Patient Received';
		} else { //failure  
			$JSON_RESULT_ARRAY = array('ERROR' => 1, 'ERROR_MSG' => 'Unable To Fetch Patient Diagnoses');
		}
