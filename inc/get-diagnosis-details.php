<?php
	...
	if(	isset($_GET['diagnosisID']) && $_GET['diagnosisID'] ) {	
		...
		$result = getDiagnosisDetails ($diagnosisID, $connection);

		if ($result) { //success
			$JSON_RESULT_ARRAY = $result;
			$JSON_RESULT_ARRAY['ERROR'] = 0;
			$JSON_RESULT_ARRAY['SUCCESS_MSG'] = 'Details Of Diagnosis Received';
		} else { //failure  
			$JSON_RESULT_ARRAY = array('ERROR' => 1, 'ERROR_MSG' => 'Unable To Fetch Diagnosis Details');
		}
