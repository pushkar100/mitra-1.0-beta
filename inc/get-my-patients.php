<?php
	...
	if(	isset($_GET['doctorID']) && $_GET['doctorID'] && 
		...
		$result = getMyPatients ($doctorID, $orderBy, $connection);

		if ($result) { //success
			$JSON_RESULT_ARRAY = $result;
			$JSON_RESULT_ARRAY['ERROR'] = 0;
			$JSON_RESULT_ARRAY['SUCCESS_MSG'] = 'Details Of My Patients Received';
		} else { //failure  
			$JSON_RESULT_ARRAY = array('ERROR' => 1, 'ERROR_MSG' => 'Unable To Fetch My Patients');
		}
